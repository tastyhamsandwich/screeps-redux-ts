...
const Repairer = {
	run: (creep: Creep) => {
		try {
			...
			console.log(`Execution Error In Function: Repairer.run(creep) on Tick ${Game.time}. Error: ${e}`);
		}
	}
}
...
function findRepairableStructures(room: Room): AnyStructure[] {
	try {
		...
		console.log(`Execution Error In Function: findRepairableStructures(room) on Tick ${Game.time}. Error: ${e}`);
		return [];
	}
}
