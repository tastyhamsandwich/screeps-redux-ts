 ...
 */
export function max<T>(ts: Iterable<T>,	map: (t: T) => number, minE: number = Number.NEGATIVE_INFINITY): T | undefined {
 ...
 */
export function min<T>(ts: Iterable<T>, map: (t: T) => number, maxE: number = Number.POSITIVE_INFINITY): T | undefined {
 ...
 */
export function maxEntry<T>(ts: Iterable<T>, map: (t: T) => number, minE: number = Number.NEGATIVE_INFINITY): { t: T; value: number } | undefined {
	let res: T | undefined = undefined
	for (const t of ts) {
		const v = map(t)
		if (v > minE) {
 ...
 */
export function minEntry<T>(ts: Iterable<T>, map: (t: T) => number, maxE: number = Number.POSITIVE_INFINITY): { t: T; value: number } | undefined {
...
export function* flatten<ITs extends Iterable<any>[]>(...tts: ITs): Generator<UnionI<ITs>, void, undefined> {
 ...
 * @yields each valid thing
 */
export function* filterIs<T, U extends T>(ts: Iterable<T>, pred: (t: T) => t is U): IterableIterator<U> {
	for (const t of ts) {
		if (pred(t)) yield t
	}
}
