...
export function getSpeedrunVisualDrawer(state = {}, startTime = Game.time, avgDuration = 100) {
	...
	return (position = { x: 1, y: 1 }) => {
		...
		for (let c = 2; c <= ctrl.level; c++) {
			...
			visual.text(
				`RCL ${c}: ${delimit(delay)} (${delimit(data.rcl[c] - data.start)}) - Avg. CP: ${Math.floor((CONTROLLER_LEVELS[c - 1] / delay) * 10) / 10
				}e/t`,
				x,
				y++,
				{ align: "left" }
		...
		visual.text(
			`Progress: ${delimit(ctrl.progress)} / ${delimit(ctrl.progressTotal)} (${Math.floor((ctrl.progress / ctrl.progressTotal) * 1000) / 10
			}%)`,
			x,
			y++,
			{ align: "left" }
